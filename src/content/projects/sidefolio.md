...
This site is fully optimized for LLMs, AI agents, and web crawlers, scoring a perfect 100% on [Is It Agent Ready?](https://isitagentready.com/www.michxymi.com) and [acceptmarkdown](https://acceptmarkdown.com/#readiness). 
...
A lot of the visual decisions borrow from modern, product-focused engineering aesthetics: clean surfaces, monochrome base, and small accents instead of shouting at you with gradients.

[JetBrains Mono](https://fontsource.org/fonts/jetbrains-mono) handles headings and code, [IBM Plex Sans](https://fontsource.org/fonts/ibm-plex-sans) covers the body copy, and together they give the site that “developer who cares but not too much” feel.
...
The UI is built on [Starwind UI](https://starwind.dev/) components, a tailwind-powered component system for Astro that handles accessibility and basic patterns, giving me [shadcn](https://ui.shadcn.com/)-style ergonomics without the React overhead.

Instead of inventing a design language from scratch, I generated the foundational color variables and theme config using its [Theme Designer](https://pro.starwind.dev/tools/theme-designer/) and layered a few custom components on top.
...
Animations and transitions are kept subtle on purpose, styled directly through CSS transitions and keyframe rules that stay out of the way.
...
Moving to Astro and Cloudflare helped me strip out tons of client-side JavaScript, while Starwind UI provided the right level of structure to construct a responsive side-navigation dashboard, all without sacrificing performance.
